...
if output=$(./cco -e MY_TEST_VAR=hello_world shell 'echo $MY_TEST_VAR' 2>&1) && [[ "$output" == "hello_world" ]]; then
...
# shellcheck disable=SC2016
if output=$(./cco -e VAR_A=alpha -e VAR_B=beta shell 'echo ${VAR_A}_${VAR_B}' 2>&1) && [[ "$output" == "alpha_beta" ]]; then
...
# shellcheck disable=SC2016
if output=$(./cco -e 'SPECIAL_VAR=hello world' shell 'echo $SPECIAL_VAR' 2>&1) && [[ "$output" == "hello world" ]]; then
...
# shellcheck disable=SC2016
if output=$(HOST_PASSTHROUGH=from_host ./cco -e HOST_PASSTHROUGH shell 'echo $HOST_PASSTHROUGH' 2>&1) && [[ "$output" == "from_host" ]]; then
...
# shellcheck disable=SC2016
if output=$(
	unset NONEXISTENT_VAR 2>/dev/null
	./cco -e NONEXISTENT_VAR shell 'echo ">${NONEXISTENT_VAR:-}<"' 2>&1
) && [[ "$output" == "><" ]]; then
...
# shellcheck disable=SC2016
if output=$(cd "$TEST_DIR" && "$OLDPWD/cco" shell 'echo $DOTENV_TEST' 2>&1) && [[ "$output" == "it_works" ]]; then
...
EOF
# shellcheck disable=SC2016
if output=$(cd "$TEST_DIR" && "$OLDPWD/cco" shell 'echo ${FIRST_VAR}_${SECOND_VAR}' 2>&1) && [[ "$output" == "one_two" ]]; then
...
# shellcheck disable=SC2016
if output=$(cd "$TEST_DIR" && "$OLDPWD/cco" -e OVERRIDE_VAR=from_flag shell 'echo $OVERRIDE_VAR' 2>&1) && [[ "$output" == "from_flag" ]]; then
